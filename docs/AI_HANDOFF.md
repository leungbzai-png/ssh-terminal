...
**Current version: v1.0.0** (Stable Release, 2026-07-04 — tag `v1.0.0`, GitHub Release, Latest). 1.0.0 is a **stabilization release only**: version bump + docs + release gate + Windows packaging on top of the v0.9.0 code and the post-v0.9.0 build-tagged integration tests. **No product code changed for 1.0.0.** Prior tags v0.4.0 / v0.5.0 / v0.7.0 / v0.9.0 unchanged; **no separate v0.6.0 or v0.8.0 tag/Release**. Advanced SSH lives in `internal/hosts/advanced.go` (non-secret config) and `internal/sshsess/{manager,tunnel,socks,diag}.go`; secret scrubbing in `internal/redact`. ProxyJump is single-level; a manual bastion is key-only (password bastions must reference a saved host).
