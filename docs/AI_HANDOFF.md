...
**Latest formal release: v1.0.0** (Stable, tag `v1.0.0`, GitHub Latest). **Dev version: 1.1.0 (in preparation, NOT released).** v1.1.0 adds the **SFTP Two-Pane Foundation** — a read-only local pane (`internal/localfs`: List/Home/Roots/Parent/Exists) beside the existing remote pane, recursive remote→local download (`sftpx.DownloadPaths`), and two-pane upload/download wiring with overwrite confirmation (`SftpExists`/`LocalExists`). Frontend: `SftpPane.vue` (local) + `SftpPanel.vue` (container, remote logic still inline); local cwd is in-memory only (never persisted). Its automated gate is green, but the **manual SFTP GUI QA (`docs/SFTP_TWO_PANE_QA.md`) has NOT been human-executed**, and **no `v1.1.0` tag/Release exists yet**. Do not claim v1.1.0 is released until a tag + Release are created. Prior tags v0.4.0–v1.0.0 unchanged; **no separate v0.6.0 or v0.8.0 tag/Release**. Advanced SSH lives in `internal/hosts/advanced.go` (non-secret config) and `internal/sshsess/{manager,tunnel,socks,diag}.go`; secret scrubbing in `internal/redact`. ProxyJump is single-level; a manual bastion is key-only (password bastions must reference a saved host).
